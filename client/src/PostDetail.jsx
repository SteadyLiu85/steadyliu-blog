            ...
            <ReactMarkdown 
              ...
              components={{
                h2: ({node, ...props}) => {
                  ...
                  return <h2 id={id} className="text-2xl md:text-3xl font-black mt-16 mb-6 pb-2 border-b-2 border-theme-border break-words" {...props} />
                },
                h3: ({node, ...props}) => {
                  const text = extractText(props.children);
                  const id = slugify(text);
                  return <h3 id={id} className="text-xl font-black mt-10 mb-4 flex items-center gap-2 before:content-['::'] before:text-theme-accent break-words" {...props} />
                },
                ...
                // === 核心修改点：重构代码块顶部栏 ===
                pre: ({node, ...props}) => {
                  // 从内部的 <code> 节点提取出 className (例如 "language-python")
                  const childProps = props.children?.props;
                  const className = childProps?.className || '';
                  const match = /language-(\w+)/.exec(className);
                  // 获取到真实的语言，如果没有则默认显示 CODE
                ...
                code: ({node, inline, ...props}) => inline ? <code className="font-mono text-sm bg-theme-base border border-theme-border px-1.5 py-0.5 rounded-sm mx-1 break-words" {...props} /> : <code {...props} />
              }}
            >
              {post.content}
            </ReactMarkdown>
