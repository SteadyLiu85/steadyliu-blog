...
const markdownComponents = {
  ...
  p: (props) => <p className="text-[17px] leading-8 mb-6 break-words" {...props} />,
  ul: (props) => <ul className="list-square list-inside space-y-2 mb-6 text-[17px] ml-4 marker:text-theme-text-primary break-words" {...props} />,
  ol: (props) => <ol className="list-decimal list-inside space-y-2 mb-6 text-[17px] ml-4 font-bold break-words" {...props} />,
  table: (props) => (
  ...
  thead: (props) => <thead className="bg-theme-base border-b-4 border-theme-border" {...props} />,
  tbody: (props) => <tbody className="divide-y-2 divide-theme-border" {...props} />,
  tr: (props) => <tr className="hover:bg-theme-hover transition-colors" {...props} />,
  th: (props) => <th className="p-4 md:p-6 font-black text-theme-text-primary border-r-2 border-theme-border last:border-r-0 uppercase tracking-widest text-sm whitespace-nowrap" {...props} />,
  td: (props) => <td className="p-4 md:p-6 font-bold border-r-2 border-theme-border last:border-r-0" {...props} />,
  code: ({ className, ...props }) => {
    ...
    if (!match) {
      return (
        <code
          className="font-mono text-[0.9em] border border-theme-border px-1.5 py-0.5 rounded-sm font-bold decoration-clone break-words"
          style={{
            backgroundColor: 'var(--color-theme-code-inline-bg)',
            color: 'var(--color-theme-code-inline-text)',
          }}
          {...props}
        />
      )
  ...
  blockquote: (props) => <blockquote className="border-l-4 border-theme-accent bg-theme-base/50 p-6 my-8 font-mono text-sm leading-relaxed text-theme-text-secondary break-words" {...props} />,
  a: (props) => <a className="text-theme-text-primary border-b-2 border-theme-text-primary hover:bg-theme-text-primary hover:text-theme-surface transition-colors break-words" {...props} />,
}
...
function PostDetail() {
  ...
  if (!post) {
    return <div className="font-mono font-black text-2xl uppercase p-20 text-center text-theme-text-primary">RUNNING...</div>
  }
        ...
        <article className="w-full lg:w-[75%] bg-theme-surface border-2 border-theme-border rounded-sm shadow-brutal min-h-screen flex flex-col min-w-0">
          <header className="border-b-2 border-theme-border p-8 md:p-12 bg-theme-base/30">
            ...
            <div className="flex flex-wrap gap-4 font-mono text-xs font-bold uppercase text-theme-text-primary">
              ...
              {post.tags?.map((tag) => (
                <Link key={tag} to={`/tags/${encodeURIComponent(tag)}`} className="border-2 border-theme-border px-2 py-1 bg-theme-base shadow-brutal-sm hover:shadow-brutal hover:-translate-y-0.5 hover:bg-theme-surface active:active-brutal-sm transition-all">
                  #{tag}
                </Link>
              ))}
            </div>
          ...
          <div className="p-8 md:p-12 text-theme-text-primary font-medium flex-1 overflow-hidden">
            <ReactMarkdown remarkPlugins={[remarkMath, remarkGfm]} rehypePlugins={[rehypeKatex, rehypeHighlight]} components={markdownComponents}>
              {post.content}
            </ReactMarkdown>
          </div>
        ...
        <aside className="hidden lg:block w-[25%] sticky top-24">
          <div className="bg-theme-surface border-2 border-theme-border p-6 rounded-sm shadow-brutal-sm flex flex-col gap-6">
            <div className="bg-theme-base border-2 border-theme-border p-4 shadow-inner">
              ...
              <div className="font-mono text-theme-accent text-sm tracking-[0.1em] whitespace-nowrap overflow-hidden">[{renderAsciiProgress()}]</div>
            </div>
            ...
            <div>
              ...
              <nav className="flex flex-col gap-2 max-h-[65vh] overflow-y-auto pr-3 scrollbar-brutal">
